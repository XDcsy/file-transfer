...
实现形态：
- `发送端`：把文件编码成图像帧并循环播放（支持全屏高可靠/低打扰水印悬浮）。
- `接收端`：先用 `ffmpeg` 抓图并归档到本地，再离线解析全部帧并重组文件。
...
- 传输模式：无回传通道，发送端循环广播帧序列，接收端通过 `manifest` 帧获取总分片和建议录制帧数后离线拼装。
- 低打扰模式：
1. 发送端自动缩小窗口并置顶（不再占用整屏）
2. 数据位采用“左右差分水印”半透明调制
3. 接收端离线阶段支持差分解码路径（计算量更高但更抗背景亮度变化）
...
3. 实际吞吐约等于：`file_bytes * fps / packets_per_loop`（界面实时估算）。
...
- 填 `ffmpeg` 路径
- 扫描或输入采集卡设备名
- 设置采集分辨率/FPS 与录制抽帧 FPS
- 设置输出目录
- 点击 `开始录制`，等待自动达到目标帧后转离线解析（或手动停止并解析）
...
- `lib/src/visual_protocol.dart`：分片、CRC、XOR parity、manifest、封包与重组
- `lib/src/visual_frame_codec.dart`：帧绘制、图像采样与解码入口
- `lib/src/frame_archive.dart`：录制帧归档格式与顺序读写
